  ...
  return (
    ...
    >
      {title || subtitle || action ? (
        <div className="flex flex-col gap-3 border-b bg-card px-4 py-4 md:flex-row md:items-start md:justify-between">
...
export function MetricTile({
  ...
  return (
    <motion.div
      ...
      className={cn(
        "surface-card rounded-lg p-4 transition-colors hover:bg-muted/40",
    ...
    >
      <div className="flex items-start justify-between gap-4">
        <div className="min-w-0">
          ...
          <div className="mt-2 font-heading text-2xl font-semibold tracking-tight text-foreground">{value}</div>
        </div>
        {icon ? <div className="grid size-10 place-items-center rounded-lg bg-primary/10 text-primary">{icon}</div> : null}
      </div>
