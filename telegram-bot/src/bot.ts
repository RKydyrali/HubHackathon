...
function prefKeyLabel(key: keyof NotificationPreferences): string {
...
function settingsText(p: NotificationPreferences): string {
  const lines = [
...
function settingsKeyboard(p: NotificationPreferences): InlineKeyboard {
  let kb = new InlineKeyboard();
  for (const key of [
    "telegram",
    "newApplications",
...
function linkedWelcomeText(username?: string): string {
  const name = username ? ` @${username}` : "";
  return `JumysAI подключён${name}. Выберите раздел ниже.`;
}

function unlinkedWelcomeText(): string {
  return [
...
function formatApplications(items: BotApplicationRow[]): string {
  if (items.length === 0) {
...
function formatNotifications(items: BotNotification[]): string {
  if (items.length === 0) {
...
async function isLinked(client: ConvexBotClient, chatId: string): Promise<boolean> {
  try {
...
async function showSettings(ctx: Context, client: ConvexBotClient, chatId: string): Promise<void> {
  await ctx.replyWithChatAction("typing");
  try {
...
async function showVacancyDetail(
  ctx: Context,
  client: ConvexBotClient,
  vacancyId: string,
): Promise<void> {
...
function ruApplyError(msg: string): string {
...
export function createBot(token: string, convexBase: string, secret: string): Bot<Context> {
  ...
  bot.command("start", async (ctx) => {
    ...
    try {
      ...
      await ctx.reply(
        linked ? linkedWelcomeText(ctx.from?.username) : unlinkedWelcomeText(),
        { reply_markup: linked ? linkedMenuKeyboard() : unlinkedMenuKeyboard() },
      );
    } catch (e) {
  ...
  bot.callbackQuery(/^apply:(.+)$/, async (ctx) => {
    ...
    await ctx.reply(
      `Ответьте на вопросы одним сообщением за раз.\n\n1/${qs.length}: ${qs[0]}`,
    );
  });
  ...
  bot.callbackQuery(/^pf:(\w+)$/, async (ctx) => {
    ...
    const key = raw as keyof NotificationPreferences;
